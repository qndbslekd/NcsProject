<%@page import="java.util.List"%>
<%@page import="jspNcsProject.dto.ProductDTO"%>
<%@page import="jspNcsProject.dao.ProductDAO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>myProductCommentList</title>
<link href="../resource/team05_style.css" type="text/css" rel="stylesheet"/>
<style>

	#nonBorder {
		border:0px;
	}
	#nonBorder tr {
		border:0px;
	}
	#nonBorder td {
		border:0px;
	}

</style>
</head>
<%
	request.setCharacterEncoding("UTF-8");
	ProductDAO dao = ProductDAO.getInstance();
	ProductDTO dto = new ProductDTO();
	String memName = (String)session.getAttribute("memName");
	// product DB테이블엔 id 말고 name들어가있으니 memName을 담아줌 

	int pageSize = 3; // 한 페이지에서 보여줄 게시글의 수
	String pageNum = request.getParameter("pageNum");
	if(pageNum == null){ // 처음 페이지를 킨 경우 null 값이 들어가니까 이 경우엔 pageNum에 1을 넣어줌 
		pageNum ="1";
	}
	
	int currPage = Integer.parseInt(pageNum);
	int startRow = (currPage - 1) * pageSize + 1;
	int endRow = currPage * pageSize;
	
	int count = 0;
	
	// 내가 쓴 글 전체 개수부터 가져오기	
	count = dao.getMyProductCommnetCount(memName);

	// 게시판에서 글 가져오기 
	List myProductCommentList = null;
	if(count > 0){// 글이 하나라도 있으면 가져오기 
		myProductCommentList  = dao.selectMyProductCommnet(startRow, endRow, memName);
	}
	
	if(memName == null){ %>
	<script>
		alert("로그인 후 이용하세요");
		window.location="loginForm.jsp";
	</script>
	<% }else{		
%>
<%-- 게시판 형태 만들기 --%>
<body>
<h3> [제품 게시판] </h3>
	<%-- 게시글이 없을 때 --%>
	<% if(count == 0){%>
		<table id="nonBorder">
			<tr>
				<td class="line">
					작성한 댓글이 없습니다.
				</td>
			</tr>
		</table>	
	<%
	}else{ %>
	<% for(int i = 0 ; i < myProductCommentList.size(); i++){
		dto = (ProductDTO)myProductCommentList.get(i);
		
		int num = dto.getRef();
		// 제품 댓글의 ref == 원본글의 num
		
		ProductDAO productDAO = ProductDAO.getInstance();
		ProductDTO productDTO = new ProductDTO();
		productDTO = dao.selectProdcut(num);
	%>
	<div onclick="location.href='../product/productContent.jsp?num=<%= productDTO.getNum()%>'">
	<table border="0" id="nonBorder">
		<tr>
			<td >
				원글제목 <%= productDTO.getName() 
				%>
			</td>
		</tr>
		<tr>
			<td>
				댓글 내용 : <%= dto.getDetail()  %>
			</td>		
		</tr>
		<tr style="border-bottom:1px solid black">
			<td>
				시간 : <%= dto.getReg() %>
			</td>
		</tr>
	</table>
	</div>
	<%}
	%>
	<%} %>
	<br />
	<div align="center">
	<%-- 게시판 목록 페이지 번호 뷰어 설정 --%>
	<%
		if(count > 0){ // if1. 게시글이 있을 때만 보여줄 것임 
			int pageCount = count / pageSize + (count % pageSize == 0 ? 0 : 1);
			int pageBlock = 10;
			int startPage = (int)((currPage-1)/pageBlock)*pageBlock + 1;
			int endPage = startPage + pageBlock - 1;
			if(endPage > pageCount){endPage = pageCount;}
			if(startPage > pageBlock){ %>
				<a href="myCommentList.jsp?pageNum=<%= startPage-pageBlock%>&option=myProductCommentList"> &lt; </a>
			<%}
			// 페이지 번호 뿌려주기
			for(int i = startPage; i <= endPage; i++){ %>
				<a href="myCommentList.jsp?pageNum=<%=i%>&option=myProductCommentList" class="pageNums"> &nbsp; <%=i %> &nbsp; </a>
			<%}
			if(endPage < pageCount){ %>
				<a href="myCommentList.jsp?pageNum=<%=startPage+pageBlock%>&option=myProductCommentList"> &gt; </a>
			<%}
			
		} // if1 끝
	
	%>
	</div>
</body>
	<%} %>
</html>